...
MM.Item.prototype.toJSON = function () {
  var data = _objectSpread({}, this._data, {
    id: this._id,
    text: this.getText()
  });
...
MM.Item.prototype.collapse = function () {
  ...
  this.timeout = setTimeout(function () {
    ...
    MM.publish('aftercollapse', _this2);
  }, 100);
  return this;
};
...
MM.Item.prototype.center = function () {
  var _this$getDOM$content$ = this.getDOM().content.getBoundingClientRect(),
...
function () {
  ...
  _babel_runtime_helpers_createClass__WEBPACK_IMPORTED_MODULE_1___default()(Note, [{
    ...
    value: function show(item) {
      ...
      var pos = item._dom.content.getBoundingClientRect();
...
MM.Map.prototype.center = function () {
  var _this$_root$getDOM = this._root.getDOM(),
      node = _this$_root$getDOM.node,
      content = _this$_root$getDOM.content;
...
MM.Keyboard.destroy = function () {
  window.removeEventListener("keydown", _this);
  window.removeEventListener("keypress", _this);
};
...
MM.Menu = {
  ...
  open: function open(x, y, target) {
    ...
    var _this$app$container$g = this.app.container.getBoundingClientRect(),
        containerX = _this$app$container$g.left,
        containerY = _this$app$container$g.top;

    var left = x - containerX;
    var top = y - containerY;

    if (left > this.app.container.offsetWidth - w) {
      left -= w;
    }

    if (top > this.app.container.offsetHeight - h) {
      top -= h;
    }
    ...
    this._dom.node.style.transform = "translate(".concat(left, "px,").concat(top, "px)");
...
MM.Layout._alignItem = function (item, side) {
  ...
  dom.content.insertBefore(dom.icon, dom.content.firstChild); // switch (side) {
  // 	case "left":
...
MM.Layout.Graph._drawHorizontalConnectors = function (item, side, children) {
  ...
  var y1 = c1.getShape().getVerticalAnchor(c1) + MM.PolyDom.getOffset(c1.getDOM().node, "top");
  var y2 = c2.getShape().getVerticalAnchor(c2) + MM.PolyDom.getOffset(c2.getDOM().node, "top");
  ...
  for (var i = 1; i < children.length - 1; i++) {
    ...
    var y = c.getShape().getVerticalAnchor(c) + MM.PolyDom.getOffset(c.getDOM().node, "top"); // 节点下的下划线
...
MM.Layout.Graph._drawVerticalConnectors = function (item, side, children) {
  ...
  for (var i = 1; i < children.length - 1; i++) {
    var c = children[i];
    var x = c.getShape().getHorizontalAnchor(c) + MM.PolyDom.getOffset(c.getDOM().node, "left");
    ctx.moveTo(x, y);
...
MM.Mouse._startDrag = function (e, item) {
  if (MM.App.disableDrag || MM.App.readonly) {
    return;
  }
